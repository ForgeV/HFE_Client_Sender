            ...
            for i in range(n):
                ...
                for j in range(n):
                    if j != i and ((M[j] >> i) & 1):
                        ...
                        I[j] ^= I[
                            i]  # такой же XOR для единичной матрицы (накопление операций, превращающих её в обратную)
...
class SecureChatClient:
    def __init__(self, host, port):
        self.host = host
        self.port = port
        self.hfe = HFE()  # Инициализация HFE с размером поля 128 бит
    ...
    def setup_gui(self):
        self.root = tk.Tk()
        self.root.title(f"Secure HFE Chat - {self.host}:{self.port}")

        # Поле ввода сообщения
        self.message_entry = tk.Entry(self.root, width=50)
        self.message_entry.pack(pady=5)

        # Поля для адреса получателя
    ...
    def receive_messages(self):
        while True:
            try:
                ...
                data = client.recv(4096)  # Увеличиваем буфер для больших сообщений
                if data:
                    try:
                        decrypted = hfe.decrypt(data)
                        self.display_message(f"[{addr[0]}:{addr[1]}] {decrypted.decode('utf-8')}")
                    except Exception as e:
                        self.display_message(f"[Error] Failed to decrypt: {str(e)}")
                client.close()
            except Exception as e:
